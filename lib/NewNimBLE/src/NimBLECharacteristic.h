...
class NimBLECharacteristic : public NimBLELocalValueAttribute {
  public:
    NimBLECharacteristic(const char*    uuid,
                         uint16_t       properties = NIMBLE_PROPERTY::NIM_READ | NIMBLE_PROPERTY::NIM_WRITE,
    ...
    NimBLEDescriptor* createDescriptor(const char* uuid,
                                       uint32_t    properties = NIMBLE_PROPERTY::NIM_READ | NIMBLE_PROPERTY::NIM_WRITE,
                                       uint16_t    maxLen     = BLE_ATT_ATTR_MAX_LEN);
...
# else
    ...
    // added in the oroginal for easy compatibality
