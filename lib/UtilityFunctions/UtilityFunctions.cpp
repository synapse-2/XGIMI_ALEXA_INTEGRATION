...
String saveServoActionHold(uint16_t newHold) {
  if (((newHold >= 1) && (newHold <= 1000))) {
    ...
    std::string str = std::format(
        "Action HOLD must be between 1 and 1000 ms 1000 ms = 1 sec; got {}",
        newHold);
    String Astr = String(str.c_str());
    debugLog(Astr);
    return Astr;
...
String saveAIoTDeviceID(String newDeviceID) {
  if (!newDeviceID.isEmpty() && (newDeviceID.length() == 36)) {
    Preferences _preferences;
    _preferences.begin(NVRAM_PERFS, false);
    ...
    std::string str = std::format(
        "Invalid Device AIoT ID, must be 36 chars;not empty got length:{}",
        newDeviceID.length());
    String Astr = String(str.c_str());
    debugLog(Astr);
    return Astr;
  ...
  String saveAIoTDeviceSECRET(String newSecret) {
    if (!newSecret.isEmpty() && (newSecret.length() == 25)) {
      Preferences _preferences;
      _preferences.begin(NVRAM_PERFS, false);
      ...
      std::string str = std::format("Invalid Device AIoT SECRET, must be 25 "
                                    "chars;not empty got length:{}",
                                    newSecret.length());
      String Astr = String(str.c_str());
      debugLog(Astr);
      return Astr;
