...
std::vector<uint16_t> BlueRC::BluetoothHID_RC::getPeerDevices() const
{
  if (BLE_server != NULL)
  {
    return BLE_server->getPeerDevices();
  }

  return std::vector<uint16_t>{};
}

NimBLEConnInfo BlueRC::BluetoothHID_RC::getPeerInfo(uint8_t handle)
{

  return BLE_server->getPeerInfoByHandle(handle);
}

bool BlueRC::BluetoothHID_RC::disconnect(uint16_t connHandle, uint8_t reason) const
{

  if (BLE_server != NULL)
  {
    return BLE_server->disconnect(connHandle, reason);
  }
