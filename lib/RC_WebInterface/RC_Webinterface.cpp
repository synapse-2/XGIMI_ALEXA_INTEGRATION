...
void RC_WebInterface::refreshGlobalJS()
{
  ...
  cloudInfo.replace("\n", "<br>\\\n");
  String blueInfo = bleRemoteDecoder.getConnectedDevicesTable();
  blueInfo.replace("\n", "<br>\\\n");
...
void RC_WebInterface::setupRoutes()
{
  ...
  /*handling diconnect all ble clients */
  _server.on("/disconnect-blueclients", HTTP_POST, [this]()
             {
               if (!checkAdminAuth())
                 return;
              ...
              _server.send(200, "plain/txt", "{ \"success\": true }");
               ...
               {
                  _server.send(200, "plain/txt",
                 "{ \"success\": false, \"message\": \"Unable to discconncet all BLE clients see weblog\" }");
               } });
